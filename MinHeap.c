...
MinHeap CreateMinHeap(){
	...
	int i ;
	for( i=0; i<=MAXSIZE; i++){
		H->Data[i] = (MinHeapElem)malloc(sizeof(MinHeapElem));	//因为坐标为0的位置不用 
	}
	H->Size = 0;
	H->Capacity = MAXSIZE;
//	memset(H,0,sizeof(HeapElem)*H->Capacity);
	H->Data[0]->Weight = MINDATA;				//定义"哨兵"为小于堆中所有可能元素的值(非实际数据) 
	return H;
}
...
MinHeap MinHeapInsert(MinHeap H,MinHeapElem item){
	...
	if( IsFullMin(H) ){	//【Compare Value】
		...
		i = H->Size;				 
		for( ;H->Data[i/2]->Weight > item->Weight ;i/=2 ){		//【Compare Value】 
			H->Data[i] = H->Data[i/2];
		}
...
/////////////////////////////////【过滤】///////////////////////////////////////////////// 
		...
		for(child=parent=1 ; parent*2 <= H->Size ; parent=child){
			...
			if( child!=H->Size )	// 如果child=size则此parent结点只有左结点了
				if( H->Data[child]->Weight > H->Data[child+1]->Weight ){   //【Compare Value】 
					child++;		//令child指向较小那个方便与del比较 
				} 
			if( H->Data[del]->Weight < H->Data[child]->Weight ){	//【Compare Value】
	...
	for(i=H->Size/2 ; i>0 ; i--){	//此层循环是向左（上）移动 
		...
		for( parent=i ; parent*2<=H->Size ; parent=child ){	//此层循环是向下过滤 
			...
			if( child!=H->Size )
				if( H->Data[child]->Weight > H->Data[child+1]->Weight )	//【Compare Value】
					child++;
			if( H->Data[i]->Weight < H->Data[child]->Weight ){			//【Compare Value】
				//根节点小于两个子结点，完成 
				break;
			}else{
